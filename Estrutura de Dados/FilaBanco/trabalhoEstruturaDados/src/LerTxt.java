...
public class LerTxt {
    ...
    //Função que le o texto e retorna uma string com todas as palavras do texto
    public String pegaTxt() throws FileNotFoundException {
        File path = new File("src/texto/ola.txt");
    ...
    //Separo as palavras em pilhas de texto
    ...
    //Junto as pilhas que possuem o mesmo nome
    ...
    //Vejo a pilha de uma palavra e a quantidade de elementos presentes
    public ArrayList<String> ocorrencia(LinkedList<Stack<Palavra>> li) {
        ...
        for (int i = 0; i < li.size(); i++) {
            pilhaAtual = li.get(i);
            palavraAtual = pilhaAtual.peek();
            lista.add(palavraAtual.getPalavra() + " - " + pilhaAtual.size());
        }
        return lista;
    }
}
