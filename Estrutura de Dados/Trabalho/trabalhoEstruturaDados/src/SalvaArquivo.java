...
public class SalvaArquivo {
    public void grava (Object ob) {
        try {
            File arquivo = arquivo = new File("src/texto/textoSalvar.txt");
    ...
    public static Object ler() {
        ...
        try {
            File file = new File("src/texto/textoSalvar.txt");
            if (file.length() == 0) {
                retorno = new LinkedList<>();
            } else {
                FileInputStream arquivo = new FileInputStream("src/texto/textoSalvar.txt");
